...
Built on Autodesk Enterprise Localization Quality Assurance (LQA) Defect Standard:
  ...
  - 0004: MISSALIGNMENT (Layout shifts & column offsets)
  - 0005: LEAD_TRAIL (Leading / trailing spacing & punctuation)
  - 0006: MISC (Missing components & general visual discrepancies)
  - 0008: SPEC_CHARACTERS (Special characters & unescaped entities)
  - 0009: TRUNCATION (Text truncation, button overflow & border clipping)
  - 0011: FONT_CONSISTENCY (Font typography mismatches)
  - 0012: COMBO_BOX_HEIGHT (Dropdown & combo-box height defects)
  - 0014: CAPTURE_BITMAP_FAILED (Browser URL capture failure)
  - 0015: BITMAP_DIFFERENCE (Pixel bitmap regression)
  - 0016: EXTENDED_CHAR_ISSUE (Accented / East Asian character corruption)
  - 0020: UNKNOWN_ERROR (Fallback)
"""
...
def detect_ui_containers(img_bgr):
    """Detects UI containers such as buttons, input fields, textareas, and card panels."""
    if img_bgr is None or img_bgr.size == 0:
        return {"buttons": [], "inputs": [], "cards": [], "all_containers": []}
    ...
    return {
        ...
        "all_containers": dedupe_boxes(buttons + inputs + cards)
    }
...
def check_baseline_has_ellipsis_in_region(gray_en, region_bbox, padding=10):
    """Checks if the baseline image contains an ellipsis in the given semantic region."""
    if gray_en is None or gray_en.size == 0:
        return False
    h, w = gray_en.shape[:2]
    ...
    if x2 <= x1 or y2 <= y1:
        return False
    sub_crop = gray_en[y1:y2, x1:x2]
    if sub_crop.size == 0 or sub_crop.shape[0] < 4 or sub_crop.shape[1] < 10:
        return False
        
    lines = extract_text_lines(sub_crop)
    for lx, ly, lw, lh in lines:
        line_crop = sub_crop[ly:ly+lh, lx:lx+lw]
        if detect_ellipsis_precise(line_crop):
            return True
            
    if detect_ellipsis_precise(sub_crop):
        return True
...
def analyze_localization_quality(img_en_bgr, img_loc_bgr):
    """
    Intelligent Computer Vision Pipeline based on Autodesk Enterprise Defect Taxonomy.
    """
    ...
    for bx, by, bw, bh in lines_loc:
        ...
        if detect_ellipsis_precise(crop_g):
            ...
            # Locate parent enclosing container
            parent_container = None
            for cbox in containers_loc["all_containers"]:
                cx, cy, cw, ch = cbox
                if cx - 5 <= bx and cy - 5 <= by and (bx + bw) <= cx + cw + 35 and (by + bh) <= cy + ch + 15:
                    if parent_container is None or (cw * ch < parent_container[2] * parent_container[3]):
                        parent_container = cbox
            
            # Differential baseline search (match by line/region)
            baseline_has_ellipsis = False
            matched_en = None
            for ex, ey, ew, eh in lines_en:
                if abs(ey - by) < 30 and abs(ex - bx) < 180:
                    matched_en = (ex, ey, ew, eh)
                    break
            
            if matched_en is not None:
                mex, mey, mew, meh = matched_en
                crop_en_matched = gray_en[max(0, mey-4): min(h, mey+meh+4), max(0, mex-4): min(w, mex+mew+4)]
                if detect_ellipsis_precise(crop_en_matched):
                    baseline_has_ellipsis = True
            
            if not baseline_has_ellipsis:
                if parent_container is not None:
                    if check_baseline_has_ellipsis_in_region(gray_en, parent_container):
                        baseline_has_ellipsis = True
                else:
                    if check_baseline_has_ellipsis_in_region(gray_en, (bx, by, min(bw + 100, w - bx), bh)):
                        baseline_has_ellipsis = True

            if baseline_has_ellipsis:
                continue

            # Free-space check: if text has plenty of margin (> 20px) inside textarea/search bar, exempt it
            is_real_truncation = False
            if parent_container is not None:
                cx, cy, cw, ch = parent_container
                container_right = cx + cw
                dist_to_border = container_right - line_right
                if dist_to_border < 20:
                    is_real_truncation = True
            else:
                is_real_truncation = True
            ...
            if is_real_truncation:
                ...
                findings.append({
                    ...
                    "remediation": "Increase container width or set 'white-space: normal;' with auto-width in CSS.",
                    "crop_baseline_b64": crop_en_b64,
                    "crop_localized_b64": crop_loc_b64
                })

    # -------------------------------------------------------------------------
    # 2. TRUNCATION (Code: 0009) - Button Text Overflow
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    if h >= 765 and w >= 820:
        btn_bar_en = img_en_bgr[700:765, 330:820]
        btn_bar_loc = img_loc_bgr[700:765, 330:820]
        if btn_bar_en.size > 100 and btn_bar_loc.size > 100:
            hsv_bar_en = cv2.cvtColor(btn_bar_en, cv2.COLOR_BGR2HSV)
            hsv_bar_loc = cv2.cvtColor(btn_bar_loc, cv2.COLOR_BGR2HSV)
            
            blue_en = cv2.inRange(hsv_bar_en, np.array([90, 150, 150]), np.array([115, 255, 255]))
            blue_loc = cv2.inRange(hsv_bar_loc, np.array([90, 150, 150]), np.array([115, 255, 255]))
            cnts_be, _ = cv2.findContours(blue_en, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            cnts_bl, _ = cv2.findContours(blue_loc, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            
            if len(cnts_be) >= 2 and len(cnts_bl) < 2:
                findings.append({
                    "id": "ERR-0006",
                    "code": "0006",
                    "category": "MISC",
    ...
    if h >= 140 and w >= 130:
        ...
        if help_crop_en.size > 50 and help_crop_en.shape[0] > 5 and help_crop_en.shape[1] > 5:
            ...
            if help_edges_en > 8.0:
                top_prof_loc_y = np.mean(gray_loc[:min(h, 140), :], axis=1)
                header_y_loc = next((y for y, val in enumerate(top_prof_loc_y) if val < 50), 0)
                y1_loc = header_y_loc + 10
                y2_loc = header_y_loc + 45
                if y2_loc <= h:
    ...
    # -------------------------------------------------------------------------
    if h >= 160:
        top_profile_en = np.mean(gray_en[:160, :], axis=1)
        top_profile_loc = np.mean(gray_loc[:160, :], axis=1)
        ...
        if len(val_en) > 0 and len(val_loc) > 0:
            ...
            if header_shift >= 20:
                findings.append({
                    ...
                    "severity": "Major",
                    "title": f"MISSALIGNMENT: Structural Header Shift ({header_shift}px)",
    ...
    # Form field indentation misalignment check (Minor)
    if h >= 400 and w >= 420:
